		...
		for (City c : controlledCities) {
			if (c.getName().equals(cityName)) {
				for (MilitaryBuilding b : c.getMilitaryBuildings()) {
					...
					if(b.isOverDrive()) {
						flag = true;
						temp=b.getMaxRecruit();
						temp2=b.getRecruitmentCost();
						b.setMaxRecruit(b.getMaxRecruit()*2);
						b.setRecruitmentCost(b.getRecruitmentCost()*2);
					}
				...
				if(flag) {
			    b.setMaxRecruit(temp);
				b.setRecruitmentCost(temp2);}
				}
			}
		}
if(listener!=null)
		listener.onRecruitUnit();
	}
